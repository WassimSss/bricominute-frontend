	...
	useEffect(() => {
		const fetchTasks = async () => {
			const promises = consumerService.jobs.map(async (job) => {
				const response = await fetch(`http://192.168.1.114:3000/job/getJobTasks/${job.idJob}`);
	...
	const allJobTasksStateRender = allJobTasksState.map((e) => {
		...
		return (
			<TouchableOpacity
				...
				style={[
					styles.cardJob,
					consumerService.jobTasks.find((task) => task.name === e.name) && styles.selectedCardJob
				]}
				onPress={() => handleAddJobTask({ name: e.name, price: e.price })}
			>
